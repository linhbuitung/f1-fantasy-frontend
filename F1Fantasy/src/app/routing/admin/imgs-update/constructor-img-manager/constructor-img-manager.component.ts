...
@Component({
  selector: 'app-constructor-img-manager',
  standalone: true,
  imports: [CommonModule, FormsModule, ReactiveFormsModule, ImageCropperComponent],
  templateUrl: './constructor-img-manager.component.html',
  styleUrl: './constructor-img-manager.component.scss'
})
export class ConstructorImgManagerComponent {
  ...
  selectedConstructor: ConstructorGetDto | null = null;
  ...
  onSearch(page: number = 1) {
    ...
    this.error = null;
    this.pageNum = page;
    this.constructorService.SearchConstructors(this.searchForm.value.query, this.pageNum, this.pageSize)
      .subscribe({
        next: (res: ConstructorSearchResultDto) => {
          this.constructors = res.items;
          this.total = res.total;
          this.loading = false;
        },
        error: () => {
          this.error = 'Failed to search constructors. Please try again.';
          this.loading = false;
        }
      });
  }

  selectConstructor(c: ConstructorGetDto) {
  ...
  async upload() {
    ...
    if (!this.selectedConstructor) {
      this.serverError = 'Select a constructor first.';
      return;
    }
    ...
    const safeName = (this.originalFile?.name ?? `constructor-${this.selectedConstructor.id}.jpg`).replace(/\.(png|jpe?g)$/i, '.jpg');
    ...
    const dto: ImgUpdateDto = { id: this.selectedConstructor.id, file };

    this.uploading = true;
    this.adminService.updateConstructorImage(this.selectedConstructor.id, dto)
      .pipe(finalize(() => (this.uploading = false)))
      .subscribe({
        next: (updated) => {
          this.successMessage = 'Constructor image updated successfully.';
          this.selectedConstructor = { ...this.selectedConstructor!, imgUrl: (updated as ConstructorGetDto).imgUrl };
        },
        error: (err) => {
